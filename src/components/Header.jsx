    ...
    <header className="bg-gray-900 text-white p-4 shadow-md fixed left-0 right-0 z-50">
      <div className="container mx-auto flex justify-between items-center">
        ...
        {session && session.user && session.user.user_metadata && (
          <p className="hidden md:block hover:text-gray-400  md:mr-[50px] md:ml-[30px] lg:ml[70px] xl:ml-[70px] flex-1 font-semibold md:text-[15px] lg:text-[16px] xl:text-[16px]">{t('Bienvenido')} {loadingUser ? <span className="animate-pulse">Cargando...</span> : session?.user?.user_metadata?.name}</p>
        )}
